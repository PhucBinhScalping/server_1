...
def main():
    ...
    # Thiết lập kích thước chuẩn cho ảnh (Độ phân giải cao để hiển thị nét trên PC)
    fig.update_layout(
        title=f"Biến động ngành {vn_now.strftime('%d-%m-%Y %H:%M:%S')}",
        paper_bgcolor='#333333', plot_bgcolor='#333333', font=dict(color='white'),
        width=1000, height=500, margin=dict(l=40, r=40, t=60, b=120),
        yaxis=dict(title='BĐ giá (%)'),
        yaxis2=dict(title='KL/TB21', overlaying='y', side='right'),
        xaxis=dict(tickangle=-45)
    )
    
    # XUẤT BIỂU ĐỒ RA DẠNG ẢNH PNG TĨNH
    try:
        fig.write_image("chart.png", engine="kaleido")
        # Thay thế bằng thẻ img, thêm class CSS responsive
        chart_element = '<img src="chart.png" alt="Biểu đồ biến động ngành" style="max-width:100%; height:auto; display:block; margin:0 auto; border-radius:8px;">'
    except Exception as e:
        print(f"Lỗi xuất ảnh: {e}. Chuyển hướng về xuất HTML dự phòng.")
        chart_element = fig.to_html(full_html=False, include_plotlyjs='cdn', config={'responsive': True})
    
    # Ghi file HTML
    with open(TEMPLATE_FILE, 'r', encoding='utf-8') as f:
        html = f.read()

    html = html.replace('{{CHART_DIEN_BIEN}}', chart_element)
...
if __name__ == "__main__":
    main()from update_index_only import get_world_index_html, get_gold_index_html
from get_index_vn import get_data_index
import pandas as pd
import requests
import plotly.graph_objects as io_go
